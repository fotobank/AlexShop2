    ...
    public function upload() {
        ...
        if ($config['disabled'] || !$config['access']['files']['upload']) {
			...
			if(file_exists($this->config['watermark'])) {
				$im = imagecreatefromstring(file_get_contents($target));
				$im_w = imagecreatefrompng($this->config['watermark']);
				$imw_x = imagesx($im_w);
				$imw_y = imagesy($im_w);
				imagecopyresampled($im,$im_w,imagesx($im)-$imw_x,imagesy($im)-$imw_y,0,0,$imw_x,$imw_y,$imw_x,$imw_y);
				$img_type = exif_imagetype($target);
				$function = "";
				switch($img_type) {
					case 1:
					$function = "imagegif";
