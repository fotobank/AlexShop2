...
class Security
{
    ...
    public function _set_error_level()
    {
     ...
     * @return bool|void
     */
    public function karo($t = false)
    {

        if(false === $this->byPass())
        {
            return false;
        }

        if(self::is_server($this->getRealIP()))
        {
            $this->send403();
        } // never htaccess ban the server IP
        ...
        {
            if((false !== $t) && (false !== self::hCoreFileChk($this->_htaccessfile, true, true)))
            {
     ...
     * @return bool
     */
    public function injectMatch($string)
    {

        $string = self::url_decoder($string);
        ...
        else
        {
            ...
            while($j <= 6)
            {
                ...
                elseif((false !== strpos($string, 'grant')) && (false !== strpos($string, 'all'))
                ...
                elseif(false !==
                    ( bool )preg_match("/(?:(sleep\((\s*)(\d*)(\s*)\)|benchmark\((.*)\,(.*)\)))/i", $string)
                )
                {
                    return true;
                }
                elseif(false !==
                    preg_match_all("/\bload\b|\bdata\b|\binfile\b|\btable\b|\bterminated\b/i", $string,
                                   $matches) > 3
                )
                {
                    return true;
                ...
                elseif(((false !==
                        ( bool )preg_match("/select|isnull|declare|ascii\(substring|length\(/i", $string))
                    && (false !==
                        ( bool )preg_match("/\band\b|\bif\b|group_|_ws|load_|concat\(|\bfrom\b/i", $string))
                ...
                $thenode          = $this->cleanString($j, self::getREQUEST_URI());
                ...
                if((false !== ( bool )preg_match('/onmouse(?:down|over)/i', $string))
                    && (2 < ( int )preg_match_all("/c(?:path|tthis|t\(this)|http:|
						(?:forgotte|admi)n|sqlpatch|,,|ftp:|(?:aler|promp)t/i",
                                                  $thenode, $matches))
                )
                {
                    return true;
                }
                elseif(((false !== strpos($thenode, 'ftp:')) && (substr_count($thenode, 'ftp') > 1))
                ...
                elseif((substr_count($string, '../') > 3) || (substr_count($string, '..//') > 3))
                {
                ...
                elseif((substr_count($string, '/') > 1)
                    && (2 <= ( int )preg_match_all("/$sqlfilematchlist/i", $thenode, $matches))
                )
                {
                    return true;
                }
                ...
                elseif(false !==
                    ( bool )preg_match("/php:\/\/filter|convert.base64-(?:encode|decode)|zlib.(?:inflate|deflate)/i",
                                       $string)
                    || false !==
                    ( bool )preg_match("/data:\/\/filter|text\/plain|http:\/\/(?:127.0.0.1|localhost)/i",
                                       $string)
                )
                {
                    return true;
                ...
                if((false !== ( bool )preg_match("/\border by\b|\bgroup by\b/i", $string))
                    && (false
                        !== ( bool )preg_match("/\bcolumn\b|\bdesc\b|\berror\b|\bfrom\b|hav|\blimit\b|offset|\btable\b|\/|--/i",
                                               $string)
                ...
                elseif((false !== strpos($string, 'waitfor') && false !== strpos($string, 'delay')
                        && (( bool )preg_match('/(:)/i', $string)))
                    || (false !== strpos($string, 'nowait') && false !== strpos($string, 'with')
                        && (false !== ( bool )preg_match("/--|\/|\blimit\b|\bshutdown\b|\bupdate\b|\bdesc\b/i",
                                                         $string)))
                )
                {
                    return true;
                ...
                elseif(((false !== strpos($string, 'create')
                        ...
                        || (false !== strpos($string, 'select')
                            && (false !==
                                ( bool )preg_match("/\bby\b|\bcase\b|from|\bif\b|\binto\b|\bord\b|union/i",
                                                   $string))))
                    && ((false !== ( bool )preg_match("/$sqlmatchlist/i", $string))
                        || (2 <= substr_count($string, ',')))
                )
                {
                    return true;
                }
                elseif((false !== strpos($string, 'union')) && (false !== strpos($string, 'select'))
                ...
                {
                    $nstring = preg_replace('/[^a-z]/i', '', self::url_decoder($string));
     ...
     * AdminLoginBypass()
     */
    public function AdminLoginBypass()
    {
        if(false !== strpos(self::getREQUEST_URI(), '.php/login'))
        {
            $this->karo(true);
    ...
    public function _REQUEST_SHIELD()
    {
        ...
        if(false !== strpos($req, '?'))
        {
            $v = $this->hexoctaldecode(strtolower(substr($req, strpos($req, '?'), strlen($req))));
            if(false !== strpos($v, '-') && ((false !== strpos($v, '?-')) || (false !== strpos($v, '?+-')))
                && ((false !== strpos($v, '-s')) || (false !== strpos($v, '-t')) || (false !== strpos($v, '-n'))
                    || (false !== strpos($v, '-d')))
            )
            {
                $this->karo(true);
        ...
        if(isset($_GET['_wp_http_referer'])
            && ($this->getPHP_SELF() == 'edit-tags.php' || $this->getPHP_SELF() == 'edit-comments.php'
                || $this->getPHP_SELF() == 'index.php')
        ...
        )
        {
            ...
            href="http://goo.gl/cL5XqN">http://goo.gl/cL5XqN</a><br />Click
            <a href=\'/index.php\'>here</a> to return to the Dashboard.');
        }

        $_requri = self::url_decoder($_SERVER['REQUEST_URI']);
        $attack  = false;
        if(substr_count($_requri, '/') > 30)
        {
            $attack = true;
        }
        if(substr_count($_requri, '\\') > 30)
    ...
    public function _QUERYSTRING_SHIELD()
    {
        ...
        {
            $qsdec = self::url_decoder($_SERVER['QUERY_STRING']);
            $q     = explode('&', $qsdec);
            for($x = 0; $x < count($q); $x++)
            {
                $v   = is_array($q[$x]) ? $this->array_flatten($q[$x]) : $q[$x];
                $val = $this->hexoctaldecode(substr($v, strpos($v, '=') + 1, strlen($v)));
                if(false !== $this->injectMatch($val) || false !== ( bool )self::blacklistMatch($val, 1))
                {
                    $this->karo(true);
    ...
    public function _COOKIE_SHIELD()
    {
        ...
        while($i < count($ckeys))
        {
            ...
            $cval = self::url_decoder(strtolower($str));
            if((is_string($ckey)))
            {
                if(false !== ( bool )self::blacklistMatch($ckey, 4)
                    || false !== ( bool )self::blacklistMatch($this->hexoctaldecode($cval), 4)
                )
                {
                    $this->karo(true);
    ...
    public function _REQUESTTYPE_SHIELD()
    {
        ...
        elseif(false === in_array($reqType, $req_whitelist, true))
        {
            $this->karo(true);
    ...
    public function _POST_SHIELD()
    {
        ...
        while($i < count($pnodes))
        {
            if((is_string($pnodes[$i])) && (strlen($pnodes[$i]) > 0))
            {
                ...
                if(false !== self::blacklistMatch($this->hexoctaldecode($pnodes[$i]), 2)
                    || false !== self::blacklistMatch(self::url_decoder($pnodes[$i]), 2)
                )
                {
                    $this->karo(true);
                    return;
    ...
    public function _SPIDER_SHIELD()
    {
        ...
        {
            if(false !== self::blacklistMatch(strtolower($this->hexoctaldecode($_SERVER['HTTP_USER_AGENT'])), 3)
            )
            {
     ...
     * @return mixed
     */
    public function hexoctaldecode($code)
    {

        $y    = is_array($code) ? implode('', $code) : $code;
        $code = (substr_count($y, '\\x') > 0) ? self::url_decoder(str_replace('\\x', '%', $y)) : $y;
        unset ($y);
        if((substr_count($code, '&#x') > 0) && (substr_count($code, ';') > 0))
        {
            $code = self::url_decoder(str_replace(';', '', $code));
            $code = self::url_decoder(str_replace('&#x', '%', $code));
        }

        return preg_replace_callback("@\\\(x)?([0-9a-f]{2,3})@", function ($x)
        {
            return chr($x[1] ? intval($x[2], 16) : intval($x[2], 8));
        }, $code);
    }

    /* function hexoctaldecode( $code ) {
           $code = ( substr_count( $code, '\\x' ) > 0 ) ? self::url_decoder( str_replace( '\\x', '%', $code ) ) : $code;
           if ( ( substr_count( $code, '&#x' ) > 0 ) && ( substr_count( $code, ';' ) > 0 ) ) {
              $code = self::url_decoder( str_replace( ';', '', $code ) );
              $code = self::url_decoder( str_replace( '&#x', '%', $code ) );
           }
     ...
     * @return mixed|string
     */
    public function cleanString($b, $s)
    {
        $s = strtolower(self::url_decoder($s));
        switch($b)
        {
            case (1):
     ...
     * @param $banip
     */
    public function htaccessbanip($banip)
    {
        if(false === self::hCoreFileChk($this->_htaccessfile, true, true))
        {
            $this->send403();
        }
        ...
        if(false !== self::hCoreFileChk($this->_htaccessfile, true, true))
        {
            ...
            $lastline = '';
            if(in_array($newline, $mybans, true))
            {
                exit();
            }
            if(in_array($limitend, $mybans, true))
            {
                $i = count($mybans) - 1;
    ...
    {
        if((!empty($fname)) && ((substr_count($fname, '.php') == 1) && ('.php' == substr($fname, -4))))
        {
            return true;
        }
        else
     ...
     * @return null|string
     */
    public function getPHP_SELF()
    {
        $filename = null;

        $filename = basename(self::setReq_uri());
        if(1 == strlen($filename) && ('/' == $filename))
        {
            $filename = '/' . $this->_default;
        } // or whatever your default file is
        preg_match("@[a-z0-9_-]+\.php@i", $filename, $matches);
        if(is_array($matches) && array_key_exists(0, $matches) && ('.php' == substr($matches[0], -4, 4))
            && (false !== self::checkfilename($matches[0]))
     ...
     * @return array
     */
    public function array_flatten($array, $preserve_keys = false)
    {
    ...
    public function byPass()
    {
        ...
        if(0 !== count($dir_bypass))
        {
            foreach($dir_bypass as $dirname)
            {
                if(false !== strpos(self::setReq_uri(), $dirname))
                {
                    return false;
                }
            }
    ...
    public function getDir()
    {
        ...
        if(false !== strpos($rootDir, '/'))
        {
            if((strlen($rootDir) > 2) && ($rootDir[0] == '/'))
            {
                ...
                if(strpos($rootDir, '.php'))
                {
                    $rootDir = '';
                }
                if((strlen($rootDir) > 0) && ('/' !== substr($rootDir, -1)))
                {
                    $rootDir = '/' . $rootDir . '/';
                }
            }
    ...
    public function check_ip($ip)
    {
        ...
        if(function_exists('filter_var') && defined('FILTER_VALIDATE_IP') && defined('FILTER_FLAG_IPV4')
     ...
     *
     */
    public function setOpenBaseDir()
    {
