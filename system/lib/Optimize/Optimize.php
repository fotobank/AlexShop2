 ...
 * @tags     php, html, xml, js, javascript, css, cleaner, clean, cleanse, clear, cruncher, optimize, optimizer, purge,
 *           obfuscate, vacuum, vacuumize
...
class Optimize
{
    ...
    public static function js(/*string*/
        $s, $is_vacuumize = true, $is_script_tag = false)
    {
        ...
        $s, $is_js = false, $is_css = false)
    {
        ...
        $s = preg_replace_callback('/<(pre|code|textarea|nooptimize)(' . $re_attrs_fast_safe . ')(>.*?<\/\\1)>/sxiSX', ['self', '_html_pre'], $s);
        ...
        return str_replace(['<nooptimize>', '</nooptimize>'], '', $s);
    }
    ...
    private static function _html_placeholder(/*string*/
        $s, $is_restore = false)
    {
        static $tags = [];
        if ($is_restore){
    ...
    private static function _html_chunks(array &$m)
    {
        ...
        if (preg_match('/^<!--(?:[\x20-\x7e]{4,60}+$|\xc2\xa0|&nbsp;)/sSX', $m[0]) &&  #\xc2\xa0 = &nbsp;
            !preg_match('/<[a-zA-Z][a-zA-Z\d]*+ [^>]*+ >/sxSX', $m[0])
        ) return $m[0];
