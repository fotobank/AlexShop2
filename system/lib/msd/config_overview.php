...
if (isset($databases['Name'][0]) && $databases['Name'][0] > '')
{
	...
	if (count($databases['Name']) == 1)
	{
		...
		$aus['db'].='<tr><td>' . Help($lang['L_HELP_COMMANDS'],"") . 'Команда перед Дампом</td><td>' . ComboCommandDump(0,$databases['db_selected_index']) . '</td></tr>';
		$aus['db'].='<tr><td>' . Help($lang['L_HELP_COMMANDS'],"") . 'Команда после Дампа</td><td>' . ComboCommandDump(1,$databases['db_selected_index']) . '</td>';
	...
	else
	{
		...
		$aus['db'].='<th>' . Help($lang['L_HELP_PRAEFIX'],"conf2") . $lang['L_PRAEFIX'] . '</th><th>' . Help($lang['L_HELP_COMMANDS'],"",11) . 'Команда перед Дампом</th><th>' . Help($lang['L_HELP_COMMANDS'],"",11) . 'Команда после Дампа</th><th>' . $lang['L_SQL_BEFEHLE'] . '</th></tr>';
...
$aus['global1'].='<tr><td>' . Help("","") . 'Файл логов:&nbsp;</td>';
...
$aus['global1'].='<tr><td>Игнорировать "ENABLE KEYS":</td><td>';
...
$aus['global3'].='</td></tr><tr><td>' . Help("","") . 'Размер SQL-Grid таблицы:&nbsp;</td>';
$aus['global3'].='<td><input type="radio" class="radio" name="interface_table_compact" value="0" ' . ( ( $config['interface_table_compact'] == 0 ) ? 'checked' : '' ) . '>&nbsp;обычный&nbsp;&nbsp;&nbsp;';
$aus['global3'].='<input type="radio" class="radio" name="interface_table_compact" value="1" ' . ( ( $config['interface_table_compact'] == 1 ) ? 'checked' : '' ) . '>&nbsp;компактный</td></tr>';
