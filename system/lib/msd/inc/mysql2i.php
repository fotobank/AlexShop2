...
if (!extension_loaded('mysql')) {
	...
	function mysql_numrows($link_identifier = NULL) {

		return mysqli_num_rows(mysql_ensure_link($link_identifier));
	}
	...
	function mysql_connect($server = MYSQL_DEFAULT_HOST, $username = MYSQL_DEFAULT_USER, $password = MYSQL_DEFAULT_PASSWORD, $new_link = false, $client_flags = 0) {
		...
		if (!$new_link) {
			...
			if (count($mysql_links)) {
				...
				if ($server === $last['server'] && $username === $last['username'] &&
                    $password === $last['password'] && is_resource($last['link'])) {
					# then take that
