    ...
    public function fetch()
    {
        ...
        if ($this->request->method('post') && !empty($_POST)){
            ...
            if (empty($product->name)){
                ...
                if (!empty($product->id)){
                    $images = $this->products->get_images(['product_id' => $product->id]);
                }
            ...
            elseif (substr($product->url, -1) == '-' || substr($product->url, 0, 1) == '-') {
                ...
                if (empty($product->id)){
                    ...
                    if ($old_bid != $product->brand_id){
                        $this->db->query('UPDATE __brands SET last_modify=now() WHERE id IN(?@)', [$old_bid, $product->brand_id]);
                    }
                ...
                if ($product->id){
                    ...
                    if (is_array($variants)){
                        ...
                        $current_variants = $this->variants->get_variants(['product_id' => $product->id]);
                        ...
                        foreach ($variants_ids as $variant_id){
                            $this->variants->update_variant($variants_ids[$i], ['position' => $variant_id]);
                            $i++;
                        }
                    }
                    ...
                    $current_images = $this->products->get_images(['product_id' => $product->id]);
                    ...
                    if ($images = $this->request->post('images')){
                        ...
                        foreach ($images as $id){
                            $this->products->update_image($id, ['position' => $i]);
                            $i++;
                        }
                    }
                    ...
                    foreach ($this->features->get_product_options(['product_id' => $product->id]) as $po){
                        $this->features->delete_option($product->id, $po->feature_id);
                    }

                    // Свойства текущей категории
                    $category_features = [];
                    foreach ($this->features->get_features(['category_id' => $product_categories[0]]) as $f){
                    ...
                    if (is_array($new_features_names) && is_array($new_features_values)){
                        foreach ($new_features_names as $i => $name){
                            ...
                            if (!empty($name) && !empty($value)){
                                ...
                                if (empty($feature_id)){
                                    $feature_id = $this->features->add_feature(['name' => trim($name)]);
                                }
                        ...
                        // Свойства товара
                        $options = $this->features->get_product_options(['product_id' => $product->id]);
                    }
            ...
            if ($product){
                // Категории товара
                $product_categories = $this->categories->get_categories(['product_id' => $product->id]);
                ...
                // Связанные товары
                $related_products = $this->products->get_related_products(['product_id' => $product->id]);
            } else {
                // Сразу активен
                $product = new \stdClass;
                $product->visible = 1;
        ...
        if (!empty($related_products)){
            ...
            $temp_products = $this->products->get_products(['id' => array_keys($r_products), 'limit' => count(array_keys($r_products))]);
            foreach ($temp_products as $temp_product){
                $r_products[$temp_product->id] = $temp_product;
            }

            $related_products_images = $this->products->get_images(['product_id' => array_keys($r_products)]);
        ...
        if (is_object($category)){
            $features = $this->features->get_features(['category_id' => $category->id]);
    ...
    private function smarty_func()
    {
        if (file_exists('backend/LicenseAdmin.php')){
            ...
            if ((!in_array($h, $l->domains) || (strtotime($l->expiration) < time() &&
                        $l->expiration != '*')) && $module != 'LicenseAdmin'){
