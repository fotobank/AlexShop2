...
class ImportAjax extends Registry
{
    ...
    private $columns_names = [
        ...
        'price' => ['price', 'цена'],
        'currency' => ['currency_id', 'currency', 'ID валюты'],
    ...
    private $internal_columns_names = [];
    ...
    private function import_item($item)
    {
        ...
        if (!empty($item['brand'])){
            ...
            if (!$product['brand_id'] = $this->db->result('id')){
                ...
                $product['brand_id'] = $this->brands->add_brand(['name' => $item['brand'], 'meta_title' => $item['brand'], 'meta_keywords' => $item['brand'], 'meta_description' => $item['brand']]);
            }
        }
    ...
    private function import_category($category)
    {
        ...
        foreach ($names as $name){
            ...
            if (!empty($name)){
                ...
                if (empty($id)){
                    $id = $this->categories->add_category(['name' => $name, 'parent_id' => $parent, 'meta_title' => $name, 'meta_keywords' => $name, 'meta_description' => $name, 'url' => $this->translit($name)]);
                }
