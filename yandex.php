...
// Категории
$categories = $registry->categories->get_categories();
print "<categories>";
...
$features = [];
$stock_filter = $registry->settings->yandex_export_not_in_stock ? '' : ' AND (v.stock >0 OR v.stock is NULL) ';

// Товары
$registry->db->query("SET SQL_BIG_SELECTS=1");
// Товары
...
$p_images = [];
foreach ($registry->products->get_images(['product_id' => $p_ids]) as $image){
...
foreach ($products as $p){
    ...
    if ($p->currency_id > 0){
        ...
        $price = round($registry->money->convert($p->price, $main_currency->id, false), 2);
        $old_price = round($registry->money->convert($p->compare_price, $main_currency->id, false), 2);
    }
    ...
    <url>" . $registry->config->root_url . '/products/' . $p->url . $variant_url . "</url>";
    print "
    <price>$price</price>
    $old_price
    ...
    if (!empty($p_images[$p->product_id])){
        foreach ($p_images[$p->product_id] as $img){
            print "<picture>" . $registry->design->resize_modifier($img, 800, 600) . "</picture>";
        }
    }

    print "
    <store>" . ($registry->settings->yandex_available_for_retail_store ? 'true' : 'false') . "</store>
    <pickup>" . ($registry->settings->yandex_available_for_reservation ? 'true' : 'false') . "</pickup>
    ...
    <description>" . htmlspecialchars(strip_tags(($registry->settings->yandex_short_description ? $p->body : $p->annotation))) . "</description>
    " . ($registry->settings->yandex_sales_notes ? "<sales_notes>" . htmlspecialchars(strip_tags($registry->settings->yandex_sales_notes)) . "</sales_notes>" : "") . "
    ";

    print "
    <manufacturer_warranty>" . ($registry->settings->yandex_has_manufacturer_warranty ? 'true' : 'false') . "</manufacturer_warranty>
    <seller_warranty>" . ($registry->settings->yandex_has_seller_warranty ? 'true' : 'false') . "</seller_warranty>
    ";

    if (!in_array($p->product_id, array_keys($features))){
        $features[$p->product_id] = $registry->features->get_product_options(['product_id' => $p->product_id]);
    }
