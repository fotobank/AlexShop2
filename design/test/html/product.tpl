                ...
                <div class = "col-lg-5">
					<a class = "fn-zoom alexshopcms btn-block relative border-a-1-info text-xs-center"
                       href = "{$product->image->filename|resize:800:600:w}" rel = "group">
						{* Промо изображение *}
                        {if $product->special}
                            <img class = "card-spec" alt = '{$product->sp_img}' title = '{$product->sp_img}'
                                 src = 'files/special/{$product->special}'/>
                        {/if}

                        {* Большое фото товара *}
                        <img itemprop = "image" class = "fn-img" src = "{$product->image->filename|resize:300:300}"
                ...
                <div class = "col-lg-5">
                    <a class = "fn-zoom alexshopcms btn-block relative border-a-1-info text-xs-center"
                       href = "design/{$settings->theme}/images/no_image.png" rel = "group">
                        ...
                        {/if}
                        {* Большое фото товара *}
                        <img class = "fn-img" src = "design/{$settings->theme}/images/no_image.png" height = "300"
				...
				<form class = "fn-variants alexshopcms row" action = "/{$lang_link}cart">
					<div class = "col-lg-6">
                        ...
                        <div class = "h4 font-weight-bold">
							<span class = "fn-price" itemprop = "price"
                                  content = "{$product->variant->price|convert}">{$product->variant->price|convert}</span>
                        ...
                        {* Рейтинг товара *}
                        <div id = "product_{$product->id}" class = "product_rating" itemprop = "aggregateRating"
                            ...
                            <span itemprop = "reviewCount"
                                  style = "display: none;">{$product->votes|string_format:"%.0f"}</span>
                        ...
                        {* Артикул товара *}
                        <div{if !$product->variant->sku} class = "hidden-xs-up"{/if}><span
                                    data-language = "{$translate_id['product_sku']}">{$lang->product_sku}</span>: <span
                                    class = "fn-sku">{$product->variant->sku}</span></div>
					...
					<div class = "col-lg-6">
						{if !$settings->is_preorder}
                            ...
                            <div class = "fn-not_preorder form-group{if $product->variant->stock > 0} hidden-xs-up{/if}">
								<button class = "btn btn-danger-outline btn-block disabled h5" type = "button"
                                        data-language = "{$translate_id['product_out_of_stock']}">{$lang->product_out_of_stock}</button>
							</div>
                            ...
                            <div class = "fn-is_preorder form-group{if $product->variant->stock > 0} hidden-xs-up{/if}">
								<button class = "btn btn-warning-outline btn-block i-preorder" type = "submit"
                        ...
                        <div class = "fn-product-amount fn-is_stock alexshopcms text-xs-center text-md-left{if $product->variant->stock < 1} hidden-xs-up{/if}">
                            ...
                            <button class = "fn-is_stock btn btn-warning i-add-cart{if $product->variant->stock < 1} hidden-xs-up{/if}"
                                    type = "submit"
                                    data-language = "{$translate_id['product_add_cart']}">{$lang->product_add_cart}</button>
	...
	$(function () {
        ...
        $('.oneclickbuy').click(function () {
            ...
            return false;
				...
				<div class = "ya-share2 m-b-2 text-xs-center text-md-left"
                     data-services = "vkontakte,facebook,twitter"></div>
			</div>
		</div>
        {* Навигация табов *}
        <ul class = "nav nav-tabs hidden-md-down" role = "tablist">
            ...
            {if $product->body}
                <li class = "nav-item">
					<a class = "nav-link active" data-toggle = "tab" href = "#annotation" role = "tab"
                       data-language = "{$translate_id['product_description']}">{$lang->product_description}</a>
				</li>
            {/if}
            ...
            {if $product->features}
                <li class = "nav-item">
					<a class = "nav-link {if !$product->body} active{/if}" data-toggle = "tab" href = "#features"
                       role = "tab" data-language = "{$translate_id['product_features']}">{$lang->product_features}</a>
				</li>
            {/if}
        ...
        <div class = "tab-content p-y-2">
			{* Описание *}
            {if $product->body}
                <button class = "btn btn-block btn-link border-a-1-info m-b-1 hidden-lg-up" type = "button"
                        data-toggle = "collapse" data-target = "#annotation" aria-expanded = "false"
                        aria-controls = "annotation">{$lang->product_description}</button>
            ...
            {if $product->features}
                <button class = "btn btn-block btn-link border-a-1-info m-b-1 hidden-lg-up" type = "button"
                        data-toggle = "collapse" data-target = "#features" aria-expanded = "false"
                        aria-controls = "features">{$lang->product_features}</button>
            ...
            {* Комментарии *}
            <button class = "btn btn-block btn-link border-a-1-info hidden-lg-up" type = "button"
                    data-toggle = "collapse" data-target = "#comments" aria-expanded = "false"
                    aria-controls = "comments">{$lang->product_comments}</button>
			<div class = "tab-pane collapse{if !$product->features && !$product->body} active{/if}" id = "comments"
                 role = "tabpanel">
				<div class = "row">
                            ...
                            {function name=comments_tree level=0}
                                {foreach $comments as $comment}
                                    ...
                                    <div class = "m-b-1" style = "margin-left:{$level*20}px">
                                        ...
                                        <div class = "p-y-05">
                                             {*Дата комментария *}
                                            <span class = "blog-data static">{$comment->date|date}
               ...
               <div class="ajax-content col-lg-5 bg-info p-y-1"
                    ...
                    data-url="ajax/comment.form.php?parent=0&type=product&object_id={$product->id}"></div>
                            ...
                            <a class = "blog-img" href = "{$lang_link}blog/{$r_p->url}">
                                ...
                                {if $r_p->image}
                                    <img class = "hidden-sm-down"
                                         src = "{$r_p->image|resize:150:150:false:$config->resized_blog_dir}"/>
                                {/if}
                            </a>
                            <div class = "h5 font-weight-bold">
                                <a class = "link-black" href = "{$lang_link}blog/{$r_p->url}"
                                   data-post = "{$r_p->id}">{$r_p->name|escape}</a>
                            </div>
                        </div>
                    </div>
                    {if $r_p@iteration % 4 == 0}
                        <div class = "col-xs-12 hidden-sm-down"></div>
                    {/if}
                {/foreach}
            </div>
