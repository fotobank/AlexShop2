...
;(function ($) {
    ...
    var Validata = {}, // Plugin Class
        ...
        messages = {
            ...
            tel: 'Invalid phone number (example: (087) 777-77-77).'
        },
        ...
        defaults = {
            ...
            errorTemplateClass: 'validata-bubble', // Class of the element that would receive error message
            errorClass: 'validata-error', // Class that would be added on every failing validation field
        ...
        // изменение окончаний
        ending = function (number, variations) {
            var ar = [2, 0, 1, 1, 1, 2];
            if (!variations) return '';
            return variations[(number % 100 > 4 && number % 100 < 20) ? 2 : ar[(number % 10 < 5) ? number % 10 : 5]];
        },
        ...
        Validator = {
            ...
            minLength: function (tmp) {
                ...
                return _length === 0 || _length >= tmp.arg || messages.minLength.replace('{count}', tmp.arg) + ending(tmp.arg, ends.o);
            },
            // Maximum lenght check
            maxLength: function (tmp) {
                return tmp.val.length <= tmp.arg || messages.maxLength.replace('{count}', tmp.arg) + ending(tmp.arg, ends.o);
            },
            // equalTo check
            ...
            maxChecked: function (tmp, self) {
                ...
                if (count === 0) return;
                return count <= tmp.arg || messages.maxChecked.replace('{count}', tmp.arg) + ending(tmp.arg, ends.o);
            },
            minChecked: function (tmp, self) {
                var cont = $(self.form.querySelectorAll('input[type=checkbox][name="' + tmp.el.name + '"]'));
                if (cont.index(tmp.el) !== 0) return; // same as above
                var count = cont.filter(':checked').length;
                return count >= tmp.arg || messages.minChecked.replace('{count}', tmp.arg) + ending(tmp.arg, ends.o);
            },
            //Selectbox check
            maxSelected: function (tmp) {
                if (tmp.val === null) return;
                return tmp.val.length <= tmp.arg || messages.maxSelected.replace('{count}', tmp.arg) + ending(tmp.arg, ends.i);
            },
            minSelected: function (tmp) {
                return ( tmp.val !== null && tmp.val.length >= tmp.arg ) || messages.minSelected.replace('{count}', tmp.arg) + ending(tmp.arg, ends.i);
            },
            // Radio
            radio: function (el) {
    ...
    Validata.prototype = {
        ...
        events: function () {
            ...
            $(this.form).submit(function (e) {
                ...
                FIELDS = this.querySelectorAll('[data-validata]');
                return self.init(e);
            });
            // real-time option control
            if (this.options.realTime === true) {
                // handle change event for form elements (without checkbox)
                $(this.form).find('[data-validata]').not('[type=checkbox]').on('change', function (e) {
                    // field to be controlled transferred to global variable
                    FIELDS = $(this);
                    return self.init(e);
                });
                // handle click event for checkboxes
                $(this.form).find('[data-validata][type=checkbox]').on('click', function (e) {
                    // fields to be controlled transferred to global variable
                    FIELDS = self.form.querySelectorAll('[data-validata][type=checkbox][name="' + this.name + '"]');
                    return self.init(e);
                });
            }
        ...
        checkFields: function (e) {
            ...
            for (var i = 0, _lengthFields = FIELDS.length; i < _lengthFields; i++) {
                ...
                var el = FIELDS[i], //current field
                    ...
                    methods = el.getAttribute('data-validata').split(','), //current field's control methods
        ...
        remoteRequest: function (ajaxOptions, cache, el, fieldName, e) {
            ...
            $(this.tmp.parent).addClass('validata-pending');
            ...
            this.xhr[fieldName] = $.ajax(ajaxOptions)
                ...
                .always(function (result) {
                    $(self.tmp.parent).removeClass('validata-pending');
                });
     ...
     * @return {object} this
     */
    $.fn.validata = function (options, _messages) {
        if ($.validataLanguage) {
            messages = $.extend(true, {}, messages, $.validataLanguage.messages);
            ends = $.extend(ends, $.validataLanguage.ends);
        }
        if (typeof _messages !== 'undefined') {
            messages = $.extend(true, {}, messages, _messages);
        }
        return this.each(function () {
            new Validata(this, options);
        });
    };
})(jQuery);