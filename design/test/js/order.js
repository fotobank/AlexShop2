...
/* Аяксовая корзина */
$(document).on('submit', '.fn-variants.alexshopcms', function(e) {
	e.preventDefault();
	var variant,
		amount;
	/* Вариант */
...
$(document).on('change', '.fn-variant.alexshopcms', function() {
...
$( document ).on( 'click', '.fn-product-amount.alexshopcms span', function() {
...
$(document).on('click', '.fn-comparison.alexshopcms', function(e){
	e.preventDefault();
	var button = $( this ),
...
$(document).on('click', '.fn-wishlist.alexshopcms', function(e){
	e.preventDefault();
	var button = $( this ),
...
$( document ).on( 'keypress', '.fn-coupon.alexshopcms', function(e) {
	if( e.keyCode == 13 ) {
		e.preventDefault();
		ajax_coupon();
	}
...
$( document ).on( 'click', '.fn-sub-coupon.alexshopcms', function(e) {
	ajax_coupon();
} );
...
$(function(){
	...
	/* Обратный звонок */
	$('.fn-callback.alexshopcms').fancybox({
		padding: 0
	});

	/* Инициализация баннера */
	$('.fn-slick-banner.alexshopcms').slick({
		infinite: true,
		speed: 500,
		slidesToShow: 1,
		slidesToScroll: 1,
	...
	$(".fn-slick-images.alexshopcms").slick({
		infinite: false,
		speed: 500,
		slidesToShow: 4,
		slidesToScroll: 1,
	...
	$(".fn-slick-carousel.alexshopcms").slick({
		infinite: true,
		speed: 500,
		slidesToShow: 6,
		slidesToScroll: 1,
	...
	if( $( '#fn-slider-price.alexshopcms' ).size() ) {
		...
		$( '#fn-slider-price.alexshopcms' ).slider( {
			range: true,
			min: range_min,
			max: range_max,
		...
		slider_all.on( 'change', function() {
			$( "#fn-slider-price.alexshopcms" ).slider( 'option', 'values', [slider_min.val(), slider_max.val()] );
			ajax_slider();
		} );
	...
	$( ".fn-search.alexshopcms" ).autocomplete( {
	...
	/* Слайдер в сравнении */
	if( $( '.fn-comparison_products.alexshopcms' ).size() ) {
	...
	$( document ).on( 'click', '.fn-show.alexshopcms a', function(e) {
		e.preventDefault();
		$( '.fn-show.alexshopcms a.active' ).removeClass( 'active' );
...
function amount_change(input, action) {
	...
	order.amount = parseInt( input.val() );
	/* в корзине */
