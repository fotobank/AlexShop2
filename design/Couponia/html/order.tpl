...
{* Список покупок *}
<table id="purchases"  class="table cart-table">
...
{* Детали заказа *}
<h2>Детали заказа</h2>
<table class="order_info table cart-table">
	<tr>
		<td>
			Дата заказа
		</td>
...
<ul id="deliveries" class=" cart-table">
    {foreach $payment_methods as $payment_method}
    	<li>
			...
			<h4><label for=payment_{$payment_method->id}>	{$payment_method->name}, к оплате {$order->total_price|convert:$payment_method->currency_id}&nbsp;{$all_currencies[$payment_method->currency_id]->sign}</label></h4>
...
<input type='submit' class="button  btn btn-primary" value='Закончить заказ'>
</form>

{* Выбраный способ оплаты *}
{elseif $payment_method}
<h2>Способ оплаты &mdash; {$payment_method->name}
<form method=post>            <div class="gap gap-small"></div><input type=submit  class="button  btn btn-primary"  name='reset_payment_method' value='Выбрать другой способ оплаты'></form>	
</h2>
<p>
{$payment_method->description}
</p>
